...
import { saveHomeSeo, type SeoSection } from '@/actions/admin-home-seo'
...
export default function HomeSeoEditor({ initialSections }: Props) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await saveHomeSeo(cleaned)
      if (result.ok) {
        setSaved(true)
      } else {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-800">
        The <strong>first section heading</strong> is used as the main H2 on the homepage. All sections appear as text blocks below the product grid. Keep the content relevant, natural, and keyword-rich for SEO.
      </div>
        ...
        {sections.map((section, idx) => (
          ...
          >
            <div className="flex items-center gap-2 px-4 py-2.5 bg-gray-50 border-b border-gray-100">
              <span className="text-gray-300"><GripVertical size={16} /></span>
              <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex-1">
                Section {idx + 1}
              </span>
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => moveUp(idx)} disabled={idx === 0}
                  className="text-xs px-2 py-1 rounded text-gray-500 hover:text-gray-900 disabled:opacity-30">↑</button>
                <button type="button" onClick={() => moveDown(idx)} disabled={idx === sections.length - 1}
                  className="text-xs px-2 py-1 rounded text-gray-500 hover:text-gray-900 disabled:opacity-30">↓</button>
            ...
            <div className="p-4 space-y-3">
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1 uppercase tracking-wide">
                  Heading {idx === 0 && <span className="text-indigo-500">(Main H2)</span>}
                </label>
                <input
                  type="text"
                  value={section.heading}
                  onChange={(e) => updateSection(idx, 'heading', e.target.value)}
                  placeholder="e.g. Buy Premium Men's Fashion Online in Bangladesh"
